# Exercício 002 — Dissecando uma Variável

* **Objetivo:** Ler uma entrada qualquer do usuário, exibir o seu tipo primitivo e analisar todas as suas propriedades textuais e numéricas (se contém apenas espaços, se é numérico, alfabético, alfanumérico, maiúsculo, minúsculo ou capitalizado).
* **Conceito Aplicado:** Função `input()`, tipo primitivo `str`, métodos de validação de string (`isspace()`, `isnumeric()`, `isalpha()`, `isalnum()`, `isupper()`, `islower()`, `istitle()`) e interpolação com f-strings.
